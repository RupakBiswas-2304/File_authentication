    ...
    def post(self, request):
        ...
        try:
            user = User.objects.create(
                f_name=f_name, l_name=l_name, email=email, phoneno=phone, hash=hash)
...
class LoginView(APIView):
    def post(self, request):
        ...
        current_time = now.year*10000000000 + now.month * 100000000 + \
...
class EditProfileView(APIView):
    def post(self, request):
        ...
        try:
            ...
            user = User.objects.filter(id=payload['id']).first()
            try:
                f_name_ = request.data['f_name']
            except:
                f_name_ = user.name
            try:
                l_name_ = request.data['l_name']
            except:
                l_name_ = user.name
            try:
